...
└─ skills/
   ...
   └─ wook-palette/                   # #21 /wook-palette (디자인 흐름의 토큰 관문)
      ├─ SKILL.md                     #   pro-max 추천/프리셋 시드 → AA 계산 → tokens+conventions
      ├─ scripts/gen_palette.py       #   결정론 생성기: HTML + WCAG 대비 계산 + tokens.css 추출
      └─ references/
         ├─ token-roles.md            #   토큰 역할 계약(base/surface/accent…) + AA 기준
         └─ presets/                  #   #22 프리셋 12종(INDEX.md·library.json, 전부 AA검증 — wook-design에서 이전)
```
